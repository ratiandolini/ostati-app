...
import {
  ...
  disputeMeta,
  hoursUntilBooking,
} from "./bookings/bookings.helpers";
  ...
  React.useEffect(() => {
    ...
    Promise.all(
      bookings.map((booking) =>
        loadBookingPaymentSummary(booking.id)
          .then((summary) => [booking.id, summary] as const)
          .catch((error) => {
      ...
      if (cancelled) return;
      setPaymentSummaries(
        entries.reduce<Record<string, BookingPaymentSummary>>((next, entry) => {
          if (!entry) return next;
          const [bookingId, summary] = entry;
          return { ...next, [bookingId]: summary };
        }, {})
      );
      if (firstFailure !== undefined) {
        reportApiError(firstFailure, { silentTransient: true });
      }
    });
